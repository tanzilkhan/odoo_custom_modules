...
class technician_creation(osv.Model, format_address):
    ...
    _commercial_partner_store_triggers = {
        'technician': (lambda self,cr,uid,ids,context=None: self.search(cr, uid, [('id','child_of',ids)], context=dict(active_test=False)),
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('technician', 'Related Company', select=True),
        'parent_name': fields.related('parent_id', 'name', type='char', readonly=True, string='Parent name'),
        'child_ids': fields.one2many('technician', 'parent_id', 'Contacts', domain=[('active','=',True)]), # force "active_test" domain to bypass _search() override
        ...
        'image_medium': fields.function(_get_image, fnct_inv=_set_image,
            ...
            store={
                'technician': (lambda self, cr, uid, ids, c={}: ids, ['image'], 10),
            },
        ...
        'commercial_partner_id': fields.function(_commercial_partner_id, type='many2one', relation='technician', string='Commercial Entity', store=_commercial_partner_store_triggers),
	    'customer_lines': fields.many2many('res.partner', 'join_retaile_customer', 'customer_list', 'reatiler_list','Customers',)
    }
    ...
    def fields_view_get(self, cr, user, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(technician_creation,self).fields_view_get(cr, user, view_id, view_type, context, toolbar=toolbar, submenu=submenu)
        if view_type == 'form':
    ...
    def _default_company(self):
        return self.env['res.company']._company_default_get('technician')
    ...
    def copy(self, default=None):
        ...
        return super(technician_creation, self).copy(default)
    ...
    def _check_ean_key(self, cr, uid, ids, context=None):
        for partner_o in self.pool['technician'].read(cr, uid, ids, ['ean13',]):
...
#   _constraints = [(_check_ean_key, 'Error: Invalid ean code', ['ean13'])]
    ...
    def update_address(self, cr, uid, ids, vals, context=None):
        ...
        if addr_vals:
            return super(technician_creation, self).write(cr, uid, ids, addr_vals, context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(technician_creation, self).unlink(cr, uid, ids, context=context)
    ...
    def write(self, vals):
        ...
        result = super(technician_creation, self).write(vals)
        for partner in self:
    ...
    def create(self, vals):
        if vals.get('website'):
            vals['website'] = self._clean_website(vals['website'])
        partner = super(technician_creation, self).create(vals)
        self._fields_sync(partner, vals)
    ...
    def _search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False, access_rights_uid=None):
        ...
        return super(technician_creation, self)._search(cr, user, args, offset=offset, limit=limit, order=order, context=context,
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=100):
        ...
        if name and operator in ('=', 'ilike', '=ilike', 'like', '=like'):
            ...
            if ids:
                ...
                return []
        return super(technician_creation,self).name_search(cr, uid, name, args, operator=operator, context=context, limit=limit)
    ...
    def view_header_get(self, cr, uid, view_id, view_type, context):
        res = super(technician_creation, self).view_header_get(cr, uid, view_id, view_type, context)
        if res: return res
...
class technician_modification(osv.Model):
    _inherit = 'sale.order'
    _columns = {
        'technician_mod': fields.many2one('technician', 'Technicians', readonly=True,
                                        states={'draft': [('readonly', False)], 'sent': [('readonly', False)]},
                                        required=True, change_default=True, select=True, track_visibility='always'),
    }

# class customer_lines(osv.Model):
# 	_name = 'customer.lines'
# 	_columns = {
# 		'name': fields.many2one('technician', 'Customer Line'),
# 		'customer_list': fields.many2one('res.partner', 'Customers'),
# 	}

class customer_addition(osv.Model):
    _inherit = 'res.partner'
    _columns ={
        'technician_lines': fields.many2many('technician', 'cs_rt_relation', 'cs_list', 'rt_list', 'Technicians'),
        # 'customer_list': fields.many2many('Customers', 'Customers'),
    }

# class technician_list(osv.Model):
#     _name = 'technician.lines'
#     _columns = {
#         'name': fields.many2one('res.partner', 'technician'),
#         'technician_list': fields.many2one('technician', 'technicians'),
#     }