...
class retailer_creation(osv.Model, format_address):
    ...
    _commercial_partner_store_triggers = {
        'retailer': (lambda self,cr,uid,ids,context=None: self.search(cr, uid, [('id','child_of',ids)], context=dict(active_test=False)),
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('retailer', 'Related Company', select=True),
        'parent_name': fields.related('parent_id', 'name', type='char', readonly=True, string='Parent name'),
        'child_ids': fields.one2many('retailer', 'parent_id', 'Contacts', domain=[('active','=',True)]), # force "active_test" domain to bypass _search() override
        ...
        'image_medium': fields.function(_get_image, fnct_inv=_set_image,
            ...
            store={
                'retailer': (lambda self, cr, uid, ids, c={}: ids, ['image'], 10),
            },
        ...
        'commercial_partner_id': fields.function(_commercial_partner_id, type='many2one', relation='retailer', string='Commercial Entity', store=_commercial_partner_store_triggers),
    ...
    def fields_view_get(self, cr, user, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(retailer_creation,self).fields_view_get(cr, user, view_id, view_type, context, toolbar=toolbar, submenu=submenu)
        if view_type == 'form':
    ...
    def _default_company(self):
        return self.env['res.company']._company_default_get('retailer')
    ...
    def copy(self, default=None):
        ...
        return super(retailer_creation, self).copy(default)
    ...
    def _check_ean_key(self, cr, uid, ids, context=None):
        for partner_o in self.pool['retailer'].read(cr, uid, ids, ['ean13',]):
...
#   _constraints = [(_check_ean_key, 'Error: Invalid ean code', ['ean13'])]
    ...
    def update_address(self, cr, uid, ids, vals, context=None):
        ...
        if addr_vals:
            return super(retailer_creation, self).write(cr, uid, ids, addr_vals, context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(retailer_creation, self).unlink(cr, uid, ids, context=context)
    ...
    def write(self, vals):
        ...
        result = super(retailer_creation, self).write(vals)
        for partner in self:
    ...
    def create(self, vals):
        if vals.get('website'):
            vals['website'] = self._clean_website(vals['website'])
        partner = super(retailer_creation, self).create(vals)
        self._fields_sync(partner, vals)
    ...
    def _search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False, access_rights_uid=None):
        ...
        return super(retailer_creation, self)._search(cr, user, args, offset=offset, limit=limit, order=order, context=context,
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=100):
        ...
        if name and operator in ('=', 'ilike', '=ilike', 'like', '=like'):
            ...
            if ids:
                ...
                return []
        return super(retailer_creation,self).name_search(cr, uid, name, args, operator=operator, context=context, limit=limit)
    ...
    def view_header_get(self, cr, uid, view_id, view_type, context):
        res = super(retailer_creation, self).view_header_get(cr, uid, view_id, view_type, context)
        if res: return res
...
class retailer_modification(osv.Model):
    _inherit = 'sale.order'
    _columns = {
        'retailer_mod': fields.many2one('retailer', 'Retailer', readonly=True,
...
# 	_columns = {
# 		'name': fields.many2one('retailer', 'Customer Line'),
...
class customer_addition(osv.Model):
    ...
    _columns ={
        'retailer_lines': fields.many2many('retailer', 'cs_rt_relation', 'cs_list', 'rt_list', 'Retailers'),
        # 'customer_list': fields.many2many('Customers', 'Customers'),
    }

# class retailer_list(osv.Model):
#     _name = 'retailer.lines'
#     _columns = {
#         'name': fields.many2one('res.partner', 'retailer'),
#         'retailer_list': fields.many2one('retailer', 'Retailers'),
#     }